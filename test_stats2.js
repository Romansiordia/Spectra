...
// App's current Slope (ISO 12099 Inverse Regression)
const slopeISO = numR / denRY;

// App's R2 calculation
const denomR = Math.sqrt(denRX * denRY);
const r = denomR > 0.000001 ? numR / denomR : 0;
const r2 = Math.pow(r, 2);

// Standard Forward Regression Slope (What Excel gives you by default if Y=NIR, X=Lab)
const slopeForward = numR / denRX;

console.log(`--- Statistics Results ---`);
console.log(`Mean Lab (X): ${meanX.toFixed(4)}`);
console.log(`Mean NIR (Y): ${meanY.toFixed(4)}`);
console.log(`Covariance (numR): ${numR.toFixed(4)}`);
console.log(`Variance Lab (denRX): ${denRX.toFixed(4)}`);
console.log(`Variance NIR (denRY): ${denRY.toFixed(4)}`);
console.log(`R2: ${r2.toFixed(4)}`);
console.log(`Slope (ISO 12099 / Inverse / App current): ${slopeISO.toFixed(4)}`);
console.log(`Slope (Standard Forward / Excel default): ${slopeForward.toFixed(4)}`);
